-- Fix "sessions" undercount in admin_campaign_funnel: it counted only
-- event_type='page_view' sessions, but page_view is emitted only on
-- directory/city pages — campaigns landing elsewhere (g-howto lands on the
-- /research article) showed 1 session while 11 were doing the quiz. Count
-- distinct sessions with ANY tagged event instead, so the entry step is
-- landing-page-agnostic.
