-- Extend admin_campaign_funnel: also split contacts by SOURCE (match vs
-- directory), so the marketing dashboard can show whether a campaign's contacts
-- came from the matching questionnaire ('match') or the therapist directory
-- ('directory'). Same shape as before, plus from_match / from_directory.
