-- שני תיקוני מדידה ב-admin_seo_overview:
--
-- 1) עמוד הבית (page='home') מקבל קבוצה משלו. עד 6/8/26 עמוד הבית לא שלח שום
--    אירוע - מי שנחת בו מחיפוש (מותג או כללי) ולחץ בתפריט נספר כאילו נחת על
--    היעד שלחץ עליו, ולכן "מאגר המטפלים" נראה כשער כניסה אורגני גדול. הוא
--    לא מקובץ תחת demand במכוון: הוא מערבב חיפושי מותג עם חיפושים כלליים,
--    והפרדה ביניהם אפשרית רק ב-Search Console.
--
-- 2) אירוע רשימה (profile_impression / filter_used) יורה לעיתים לפני ה-page_view
--    של אותו עמוד, ואז נחיתה על עמוד עיר נספרה כנחיתה על המאגר. כשהנגיעה
--    הראשונה היא אירוע רשימה ותוך 5 שניות מגיע page_view - ה-page_view הוא
--    הנחיתה האמיתית. (מדידה: העביר 4 סשנים מ-directory ל-city/specialty.)
