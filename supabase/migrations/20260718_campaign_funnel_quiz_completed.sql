-- Extend admin_campaign_funnel with quiz_completed (distinct sessions that
-- finished a questionnaire, per utm_campaign) so the marketing dashboard can
-- show the full paid funnel: clicks -> sessions -> quiz -> profile -> contact.
-- Answers "do paid visitors fill the quiz and then not reach out?" directly.
