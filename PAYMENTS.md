...
| | תיאור | חומרה |
|---|--------|--------|
| F1 | 🟠 **עדיין פתוח (בדיקה חיה 17/6/26):** הביטול נכשל שוב עם "Customer item not found" על הזמנה אמיתית — ה-fallback ל-`Customer.ID` הפנימי לא פתר. מה שכן עובד: שלב האימות (re-read) מונע סימון-מבוטל כוזב ומחזיר 502. צריך לברר מול Sumit את קריאת הביטול הנכונה (אולי לאחסן את ה-CustomerID הפנימי בעת היצירה ולבטל לפיו). ביטול ידני ב-Sumit UI תמיד עובד. | 🟠 |
