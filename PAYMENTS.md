...
|-----|-----------|---------|--------------|
| שאלון מטופלים | ₪30 + 18% מע"מ = ₪35.40 | חד-פעמי, אחרי 5 שאלונים חינמיים | `chargeQuizPayment` |
...
מטפל פונה למייל/טלפון של החברה (admin@getmentalytics.com / 055-993-1403) → אדמין מבטל דרך התהליך לעיל.
...
5. **לרשום את ההחזר בספרים:** אדמין → `/admin/therapists` → "בטל מנוי + הורד" →
   לסמן "בוצע גם החזר כספי" ולמלא את הסכום ברוטו (כפי שהוחזר לכרטיס), התאריך
   ומספר מסמך הזיכוי. נוצרת שורת הוצאה בקטגוריית `refund_*` + שורת audit.
   אם המנוי כבר בוטל קודם — אפשר להקליד את ההוצאה ישירות ב-`/admin/finance`.

**הזיכוי עצמו נשאר ידני בכוונה:** חיובי החידוש החודשיים נגבים אצל Sumit ולא
נרשמים ב-`payments`, כך שהסכום שנגבה בפועל ידוע רק למסמכים ב-Sumit, ורק שם
אפשר לזכות סכום חלקי עם המע"מ הנכון. יש ל-Sumit API לזיכוי (`POST
/billing/payments/charge/` עם `UnitPrice` שלילי ו-`SupportCredit: true`), אבל
בנפח הנוכחי הוא מוסיף סיכון בלי לחסוך עבודה.

### למה ההחזר לא מסומן ב-`payments.status = 'refunded'`
הסטטוס קיים בסכמה אבל **בכוונה לא נכתב**. ההכנסות נקראות חיות מ-`payments`
(`status='completed'`), אז סימון כ-`refunded` היה מפחית את הכסף פעמיים — גם
מההכנסה וגם כשורת ההוצאה — ובנוסף היה מוחק את המטפל מחישוב חודש הגיוס וה-CAC
ב-`admin-crm/finance`, שסופר תשלומי `subscription` שסטטוסם `completed`.
מקור אמת אחד לכל צד: `payments` לכסף שנכנס, `expenses` לכסף שיצא.
...
| | תיאור | חומרה |
|---|--------|--------|
| F1 | ✅ **נפתר ואומת חי (17/6/26):** הבאג — מזהה הוראת-הקבע נשלח בשדה `ID` במקום `RecurringCustomerItemID`. התיקון: `cancelSubscription` שולח `Customer.ExternalIdentifier` + `RecurringCustomerItemID` (בלי fallback). אומת חי על הזמנה אמיתית (קריאה החזירה Status 0, וההוראה עברה ל-Status 1 = מבוטל). שלב האימות (re-read) נשמר. | ✅ |
...
- **טלפון/מייל עסקי (לבקשות ביטול/החזר מלקוחות):** admin@getmentalytics.com | 055-993-1403
