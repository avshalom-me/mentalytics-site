  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (res.status === 401) {
          setError("החשבון שלכם עדיין לא מקושר למרכז. אם אתם מקימים את חשבון המרכז - פתחו את הקישור שבמייל \"ברוכים הבאים\" שקיבלתם אחרי התשלום, הוא מקשר אוטומטית תוך דקה. אם מישהו אחר כבר מנהל את המרכז - בקשו ממנו להוסיף את הכתובת שלכם ב\"צוות הניהול\" בפורטל. לא מסתדר? כתבו לנו עם שם המרכז ונקשר ידנית תוך יום עסקים.");
        } else if (!json.ok) {
