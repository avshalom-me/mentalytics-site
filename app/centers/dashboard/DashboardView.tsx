...
// (התנתקות, עריכה, הוספת מטפל) - הן שייכות לבעל החשבון, לא לצופה.
export default function CenterDashboardView({ data, preview = false, justCreated = false, logout }: {
  data: PortalData;
  preview?: boolean;
  justCreated?: boolean;
  logout?: () => void;
  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-8 pb-20" dir="rtl" style={{ fontFamily: "'Heebo', sans-serif" }}>
      ...
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        ...
        {!preview && <button onClick={logout} className="inline-flex items-center gap-1.5 rounded-full border border-stone-300 px-4 py-1.5 text-sm font-semibold text-stone-600 hover:bg-stone-50">
      ...
      {/* הזמנת מטפלים במייל (מסלול 1) - כל מטפל ממלא את הפרופיל שלו בעצמו.
          לא ב-preview: הפאנל קורא לסשן של המרכז ומפנה ל-/centers/login כשאין
          כזה, כלומר צפייה מהאדמין הייתה נזרקת החוצה מיד. */}
