  ...
  return (
    <article className="space-y-5">
      {blocks.map((block, i) => {
        if (block.startsWith("## ")) {
          return (
            <h2 key={i} className="text-2xl font-black text-stone-900 pt-4">
        ...
        if (block.startsWith("### ")) {
          return (
            <h3 key={i} className="text-xl font-bold text-stone-900 pt-2">
        ...
        if (isList) {
          return (
            <ul key={i} className="list-disc space-y-1 pr-6 text-stone-700 leading-8">
              {lines.map((l, j) => (
