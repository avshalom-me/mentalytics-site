...
function TrendChart({ trend }: { trend: TrendPoint[] }) {
  ...
  const data = trend.map(t => ({ ...t, week: formatHebrewBucketLabel(t.week_start) }));
  return (
          ...
          {(r.therapist_data.growers || r.therapist_data.decliners) && (
            <section className="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <TherapistTrendList rows={r.therapist_data.growers ?? []} direction="up" />
              </div>
              <div>
                <h3 className="text-sm font-black text-[#0F5468] mb-2">מטפלים בירידה</h3>
                <TherapistTrendList rows={r.therapist_data.decliners ?? []} direction="down" />
              </div>
            </section>
          )}
...
export default function WeeklyReportsPage() {
  ...
  async function runNow() {
    ...
    const json = await triggerReport("/api/admin-trigger-weekly-report");
    if (json.ok) {
