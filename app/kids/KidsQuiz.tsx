...
import {
  PAGES,
  GA_GRADES, BV_GRADES, ZY_GRADES,
  gg, acadGg, devAgeOk, q9AdhdActive,
  traitNeeds, skipPage, nextPid, prevPid,
  updAQ, updMQ, updOQ, updTQ, updPQ, updEQ, updBQ, updLSAS,
  computeBehPlan, updAddict,
...
// Who is answering. "parent" is the questionnaire as it always was; "counselor"
// adds the school's angle inside each branch, a refinement screen before the
// report, and the committee map on it - and takes the therapist search away,
// since a counsellor steering parents to a named private practitioner is not a
// position to put her in. Every difference is gated on this one value.
export type Audience = "parent" | "counselor";
    ...
    <div>
      ...
      <ul className="mb-5 flex flex-col gap-2">
        {[
          ...
          ["⏱️", "2-4 דקות", "זה הזמן שלוקח לרוב ההורים"],
...
// ── p-mq-sui ─────────────────────────────────────────────────────────────────
function PageMQSui({ A, setA, onNext, onBack, audience }: { A:Ans; setA:(a:Ans)=>void; onNext:(a:Ans)=>void; onBack?:()=>void; audience?: Audience }) {
  return (
    <div>
      <Card>
...
// ── p-acad ────────────────────────────────────────────────────────────────────
type PageProps = { A: Ans; setA: (a: Ans) => void; onNext: (a?: Ans) => void; onBack?: () => void; items?: Record<string, any[]> | null; audience?: Audience };
...
function PageAcad({ A, setA, onNext, onBack, items, audience }: PageProps) {
  const grp = acadGg(A);
...
// ── p-beh ─────────────────────────────────────────────────────────────────────
function PageBeh({ A, setA, onNext, onBack, audience }: { A:Ans; setA:(a:Ans)=>void; onNext:(a:Ans)=>void; onBack?:()=>void; audience?: Audience }) {
...
function PageSoc({ A, setA, onNext, onBack, items, audience }: { A:Ans; setA:(a:Ans)=>void; onNext:(a:Ans)=>void; onBack?:()=>void; items?: Record<string, any[]> | null; audience?: Audience }) {
...
function PageResult({ A, score, scoreAlgo, restored, scoreError, onRetryScore, onRestart, audience }: { A: Ans; score: KidsScoreResult | null; scoreAlgo?: string | null; restored?: boolean; scoreError: boolean; onRetryScore: () => void; onRestart: () => void; audience?: Audience }) {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    try {
      if (audience !== "counselor") sessionStorage.setItem("kids_view_v1", JSON.stringify({ ts: Date.now(), view, selectedKey }));
    } catch {}
  }, [view, selectedKey]);
  ...
  return (
    <div id="kids-results-card">
      ...
      {hasAnyFindings && audience !== "counselor" && (
      ...
      <div className="print:hidden" data-html2canvas-ignore="true">
        {audience !== "counselor" && <QuizFeedbackBox quizType="kids" />}
      </div>
    </div>
  );
}
...
// ── Main component ────────────────────────────────────────────────────────────
export default function KidsQuiz({ audience = "parent" }: { audience?: Audience }) {
  const [step, setStep] = useState<string>("p-consent");
  const [A, setA]       = useState<Ans>({});
  // The counsellor rubric is free (docs/school-questionnaire-plan.md, section 9),
  // so there is no usage gate to wait for.
  const [usageAllowed, setUsageAllowed] = useState<boolean | null>(audience === "counselor" ? true : null);
  const [paymentRequired, setPaymentRequired] = useState(false);
  // Counsellor drafts - see counselor.tsx. Nothing here runs for parents.
  ...
  useEffect(() => {
    if (step !== "p-result" || audience === "counselor") return;
    try {
  ...
  // Counsellor drafts: autosave from the first real screen on, resume from the
  // list on the consent screen. The audience flag rides inside the answers so
  // that skipPage can show p-refine without a second source of truth.
  useEffect(() => {
    if (audience !== "counselor" || !draftId || step === "p-consent") return;
  ...
  const pageProps = { A, setA, onNext: goNext, onBack: canGoBack ? goBack : undefined, items: kidsItems, audience };
        ...
        <div className="flex items-center justify-between mb-2">
          {step === "p-consent" ? (
            <div className="w-full text-center mb-1">
              ...
              <p className="text-sm" style={{ color: "var(--muted)" }}>{audience === "counselor" ? "שאלון מסייע להפניות - לצוותי חינוך" : "שאלון הפניה לטיפול - ילדים ונוער"}</p>
            </div>
          ) : (
            <>
              <span className="text-xl font-extrabold" style={{ color: "var(--teal)" }}>טיפול חכם</span>
              {step !== "p-result" && (
                <span className="text-xs px-3 py-1 rounded-full" style={{ color: "var(--muted)", background: "var(--surface-2)" }}>{audience === "counselor" ? "שאלון לצוותי חינוך" : "שאלון ילדים / מתבגרים"}</span>
              )}
            </>
          )}
        </div>
