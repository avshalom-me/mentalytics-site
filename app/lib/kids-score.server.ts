...
function computeResults(A: Ans): KidsBox[] {
  ...
  if ((A.q1 || 0) >= 3) {
    ...
    if (grp === "zy") {
      emoStandalones.push({ cls: "info", txt:
        "📌 כלים ופרקטיקות להפחתת מתח בנוער:\n\n" +
        "🟢 קרקוע (5-4-3-2-1): לזהות 5 דברים שרואים, 4 שאפשר לגעת בהם, 3 שומעים, 2 מריחים, 1 טועמים. עוזר להחזיר תשומת לב להווה ברגעי לחץ.\n\n" +
        "🟢 נשימה מבוקרת (Box Breathing): שאיפה 4 שניות, החזקה 4 שניות, נשיפה 4 שניות, החזקה 4 שניות. לחזור על המחזור 4-6 פעמים.\n\n" +
        "🟢 הרפיית שרירים פרוגרסיבית: להדק כל קבוצת שרירים למשך כ-5 שניות ואז לשחרר - מהכפות עד לפנים. עוזר במיוחד לפני שינה.\n\n" +
        "🟢 רישום מחשבות (Thought Record): לכתוב את המחשבה המלחיצה, לזהות את הרגש שהיא מעוררת ואת עוצמתו, ולחפש פרשנות אלטרנטיבית מאוזנת יותר.\n\n" +
        "🟢 היגיינת שינה: שעת שינה קבועה, צמצום מסכים שעה לפני השינה, חדר חשוך וקריר. שינה לקויה מעצימה חרדה ומצב רוח ירוד.\n\n" +
        "🟢 פעילות גופנית סדירה: 30 דקות ביום (ריצה, חדר כושר, רכיבה, שחייה) - אפקט מוכח מחקרית בהפחתת חרדה ודיכאון.\n\n" +
        "🟢 צמצום ממריצים: הפחתה של קפאין, אנרגיה, ניקוטין וקנאביס - כולם מעצימים תסמיני חרדה.\n\n" +
        "🟢 אפליקציות מבוססות-מחקר: Headspace, Calm, או Smiling Mind - סדנאות מודעות קצרות ביום-יום."
      });
    } else {
      emoStandalones.push({ cls: "info", txt:
        "📌 כלים ופרקטיקות להפחתת מתח בילדים:\n\n" +
  ...
  // BMI - חריגה מהטווח הסטנדרטי. אצל ילדים BMI נקבע לפי גיל ומגדר; הסף כאן
  // (18.5–24.9) הוא טווח המבוגרים בלבד וצריך להיחשב כסקירה ראשונית. אם חריג -
  // מפנים לדיאטנ/ית קלינית (פרופסיונל, סינון קשיח) ולרופא/ת המשפחה (חיצוני).
  const bmiNum = Number(A._bmi) || 0;
  if (bmiNum > 0 && (bmiNum < 18.5 || bmiNum > 24.9)) {
    emoStandalones.push({ cls: "purple", txt: "📊 BMI חורג מהטווח הסטנדרטי - מומלץ בירור רפואי ותזונתי" });
    emoStandalones.push({ cls: "info",   txt: "✅ הפנייה: דיאטנית קלינית" });
...
function computeDevResults(A: Ans): KidsBox[] {
  ...
  if (A.dev_toilet === "כן") {
    ...
    if (ttype === "א") {
      ...
      box("info", "✅ הפנייה: טיפול באנקופרזיס בשילוב הדרכת הורים");
