...
export async function loadCenterHealth(): Promise<CenterHealthReport> {
  ...
  const centers: CenterHealth[] = withReadiness.map((c) => {
    ...
    // 2. אין וואטסאפ: 71% מהפניות באתר הן בוואטסאפ. כרטיס ישות בלי נייד מציג
    //    רק כפתור הודעה; עמוד מרכז בלי נייד מציג רק חיוג.
    if (!whatsapp && (isEntity || x?.public_page_enabled)) {
      flags.push({
        key: "no_whatsapp",
        severity: isEntity ? "high" : "normal",
        owner: "center",
        label: "אין מספר נייד לוואטסאפ",
        detail: isEntity
          ? "הטלפון הציבורי נייח, ולכן בכרטיס המרכז בתוצאות השאלון יש רק כפתור הודעה, בלי וואטסאפ."
          : "הטלפון הציבורי נייח, ולכן בעמוד המרכז אין כפתור וואטסאפ.",
        question: "לבקש מספר נייד לוואטסאפ עסקי. רוב הפונים באתר בוחרים בוואטסאפ, ומעטים שולחים הודעה.",
        emailParagraph:
          "לפניות מטופלים חסר למרכז מספר נייד לוואטסאפ עסקי. רוב הפונים באתר בוחרים לפנות בוואטסאפ, וכרגע מוצגים להם רק חיוג או שליחת הודעה. אפשר להוסיף את המספר בפורטל, בעריכת פרופיל המרכז.",
      });
    }
