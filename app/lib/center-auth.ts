import "server-only";
import type { NextRequest } from "next/server";
import { supabaseAdmin } from "@/app/lib/supabaseAdmin";

// אימות פורטל המרכז - משותף לכל נתיבי /api/center-portal/*.
//
// מ-9/9/2026 הגישה נקבעת לפי center_members: כל שורה היא חשבון Supabase Auth
// שמורשה לנהל את המרכז, ומרכז יכול להחזיק כמה כאלה. therapy_center_accounts.user_id
// נשאר כ"החשבון הראשי" - זה שקישר את המרכז - לתצוגה באדמין ולהגנה מפני נעילה
// (אי אפשר להסיר אותו מהפורטל, רק אדמין). ההסרה של כל חבר אחר היא מחיקת שורה.
//
// אין claim-by-email, גם לא לחברים: המייל אינו מאומת בהרשמה (auto-confirm),
// ולכן "יש לי אימייל תואם" אינו הוכחת בעלות. חבר חדש נוסף רק על ידי חבר קיים
// ומאומת (POST /api/center-portal/members) או על ידי אדמין. זהו אותו עיקרון
// שאוכף therapist-claim.ts עבור פרופילי מטפלים חיים.

export type PortalCenter = {
  id: string;
  name: string;
  status: string;
  user_id: string | null;
  email: string | null;
  payer_email: string | null;
  billing_track: string | null;
  price_per_therapist: number | null;
  therapist_count: number | null;
  fixed_monthly_price: number | null;
  num_locations: number | null;
  billing_starts_at: string | null;
  slug: string | null;
  public_page_enabled: boolean | null;
  public_description: string | null;
  public_managers: string | null;
  public_city: string | null;
  public_website: string | null;
  public_phone: string | null;
  public_founded_year: number | null;
  public_team_size: number | null;
  public_address: string | null;
  public_hours: string | null;
  public_accessibility: string | null;
  public_director: unknown;
  public_faq: unknown;
  logo_path: string | null;
  team_members: unknown;
  gallery: unknown;
  // מי מחזיק את הסשן הנוכחי. נקבע רק ב-resolveCenter; חסר ב"צפייה בתור מרכז"
  // מהאדמין, ולכן אופציונלי. משמש לייחוס ביומן ולזיהוי הפרופיל האישי.
  acting_user_id?: string;
  acting_email?: string;
};

// רשימת העמודות שהפורטל צריך. מיוצאת כדי שגם "צפייה בתור מרכז" באדמין
// תטען בדיוק את אותן עמודות ולא תיפול על שדה חסר.
export const PORTAL_CENTER_COLS =
  "id, name, status, user_id, email, payer_email, billing_track, price_per_therapist, therapist_count, fixed_monthly_price, num_locations, billing_starts_at, slug, public_page_enabled, public_description, public_managers, public_city, public_website, public_phone, public_founded_year, public_team_size, public_address, public_hours, public_accessibility, public_director, public_faq, logo_path, team_members, gallery";

export async function resolveCenter(req: NextRequest): Promise<PortalCenter | null> {
  const token = req.headers.get("authorization")?.replace("Bearer ", "");
  if (!token) return null;

  const { createClient } = await import("@supabase/supabase-js");
  const supabaseAuth = createClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
  );
  const { data: { user }, error } = await supabaseAuth.auth.getUser(token);
  if (error || !user) return null;

  // חברות היא מקור האמת. UNIQUE(user_id) בטבלה מבטיח שורה אחת לכל היותר,
  // ולכן maybeSingle כאן דטרמיניסטי ולא תלוי בסדר.
  const { data: membership } = await supabaseAdmin
    .from("center_members")
    .select("center_id")
    .eq("user_id", user.id)
    .maybeSingle();
  if (membership) {
    const { data: center } = await supabaseAdmin
      .from("therapy_center_accounts")
      .select(PORTAL_CENTER_COLS)
      .eq("id", membership.center_id)
      .maybeSingle();
    if (center) {
      return { ...(center as PortalCenter), acting_user_id: user.id, acting_email: user.email ?? undefined };
    }
  }

  // ── אין fallback ל-claim-by-email (סגירת פרצת השתלטות) ──────────────────
  // בעבר, אם לא נמצא מרכז לפי user_id, קושר כאן אוטומטית מרכז פעיל שכתובת
  // המייל שלו תואמת למי שנרשם. המייל אינו מאומת בהרשמה (auto-confirm), ולכן
  // מי שנרשם ראשון עם המייל של מרכז פעיל היה משתלט עליו: עריכת פרטי הקשר של
  // מטפלי המרכז (הסטת לידים), הוספת פרופילים למנוי המרכז, ונעילת הבעלים
  // האמיתי בחוץ. זו בדיוק הפרצה שנסגרה למטפלים ב-therapist-claim.ts, שם
  // פרופיל חי/משלם לעולם לא נתפס אוטומטית אלא מקושר ע"י אדמין.
  //
  // ── קישור ידני של חשבון למרכז (אדמין) ──────────────────────────────────
  // הדרך הרגילה היא קישור ההצטרפות (claim-account) לחשבון הראשון, ואחריו
  // "צוות הניהול" בפורטל לכל חשבון נוסף. כשצריך לקשר ידנית:
  //   1. Supabase → Authentication → Users → לאתר לפי המייל → להעתיק UUID
  //      (ודאו שזה באמת מי שביקש גישה - זהו שלב האימות היחיד).
  //   2. SQL Editor:
  //        insert into public.center_members (center_id, user_id, email)
  //        values ('<CENTER_ID>', '<AUTH_USER_UUID>', '<EMAIL>')
  //        on conflict do nothing;
  //      ואם זה החשבון הראשון של המרכז (user_id ריק), גם:
  //        update public.therapy_center_accounts
  //           set user_id = '<AUTH_USER_UUID>', updated_at = now()
  //         where id = '<CENTER_ID>' and user_id is null;
  // עד אז הדשבורד מציג "החשבון עדיין לא מקושר" עם ההסבר איך להתקשר.
  return null;
}
