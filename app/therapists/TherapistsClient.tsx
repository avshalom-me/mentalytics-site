...
export type PublicTherapist = {
   ...
   */
  trackable?: boolean;
  /** וואטסאפ עסקי של מרכז (ישות או כרטיס מסונתז), אחרי כלל הנפילה לאחור של phone.ts. */
  center_whatsapp?: string | null;
  /** מזהה חשבון המרכז - לרישום לחיצה על כרטיס מסונתז, שאין לו שורת מטפל. */
  ...
  return (
    ...
    >
      <CardLink href={cardHref} className="block">
        ...
        <div className="px-5 pt-4 pb-3">
          ...
          {/* למרכז אין שורת תואר. היא הציגה את שני סוגי המטפלים הראשונים
              ברשימה - אצל אחד המרכזים "עו"ס קליני · מטפל מיני", מתוך תשעה שהוא
      ...
      <div className="px-5 pb-5 flex flex-wrap items-center gap-2 border-t border-stone-100 pt-3">
        ...
        {/* וואטסאפ של המרכז (15/9/26): הוואטסאפ העסקי, או הטלפון לחיוג אם הוא
            נייד. ראשון, כמו אצל מטפל. ישות רושמת לחיצה על שורת המטפל שלה;
            כרטיס מסונתז - על חשבון המרכז (center_contact_click). */}
        {isCenter && !unavailable && waLinkForCenter(t.center_whatsapp) && (
          <CenterWhatsAppLink
            entityId={t.trackable !== false ? t.id : undefined}
            centerId={t.center_account_id ?? undefined}
            href={waLinkForCenter(t.center_whatsapp)!}
            source="directory"
            className="inline-flex items-center gap-1.5 rounded-full bg-[#128C42] px-4 py-2 text-[13px] font-bold text-white hover:bg-[#0F7A39]">
